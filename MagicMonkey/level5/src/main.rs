...
fn process_file(input_path: &Path) -> io::Result<()> {
    ...
    for _ in 0..currencies {
        let mut coin_values: Vec<AvailableCoins> = lines
            .next()
            .unwrap()?
            .split_whitespace()
            .map(|s| {
                ...
                AvailableCoins {
                    amount: d[0].parse().unwrap(),
                    coin_value: d[1].parse().unwrap(),
                }
            })
            .collect();
        ...
        let available_coin_values: Vec<u32> = coin_values.iter().map(|c| c.coin_value).collect();
        let lcm = least_common_multiple(&available_coin_values);

        let mut still_available = coin_values.clone();
        let mut used: Vec<AvailableCoins> = Vec::new();

        for amount in amounts {
            let upper_bound = amount.saturating_sub(2 * lcm);

            while still_available.len() > 0 {
                let last_coin = still_available.last().unwrap();
                let number_of_max_coins = if upper_bound > 0 { upper_bound / last_coin.coin_value } else { 0 };
                upper_bound -= number_of_max_coins * last_coin.coin_value;
